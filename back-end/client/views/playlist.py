...
@csrf_exempt
async def add_song_to_playlist(request, playlist_id):
    if request.method == "PATCH":
        try:
            body = json.loads(request.body)
            ...
            title = body.get("title")
            image_album = body.get("imageAlbum")
            new_song_id = body.get("songId")  # Thêm một bài hát duy nhất
            deleted = body.get("deleted")
            ...
            if new_song_id:
                if not isinstance(playlist.songs, list):
                    playlist.songs = []
                if new_song_id not in playlist.songs:
                    playlist.songs.append(new_song_id)
